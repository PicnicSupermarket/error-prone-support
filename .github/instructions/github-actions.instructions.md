...
Declare `permissions: {}` at workflow level, granting nothing by default.
Every job must declare its own complete `permissions` block for exactly what it
needs. Document every permission scope beyond `contents: read` with a trailing
comment explaining why it is needed; `contents: read` itself needs no comment.
