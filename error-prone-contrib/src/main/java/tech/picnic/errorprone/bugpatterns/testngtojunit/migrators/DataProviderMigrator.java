...
public class DataProviderMigrator implements Migrator<Void> {
    @Override
    public Optional<SuggestedFix> createFix(MethodTree methodTree, Void dataValue, VisitorState state) {

        return Optional.empty();
    }

    @Override
    public boolean canFix(MethodTree methodTree, Void dataValue, VisitorState state) {
    ...
    private static Optional<SuggestedFix> fixValueFactory(
        ...
        TestNGMigrationContext.MigrationState migrationState =
                dataProviderMigration.isPresent() ? TestNGMigrationContext.MigrationState.MIGRATED : TestNGMigrationContext.MigrationState.CANNOT_MIGRATE;
