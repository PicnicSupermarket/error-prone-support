  ...
  void identification() {
    CompilationTestHelper.newInstance(EmptyReactivePublisher.class, getClass())
        .addSourceLines(
            ...
            "    Mono.empty().subscribe(null, t -> {});",
            "",
            "    Mono.empty().then();",
            "    Mono.empty().doOnError(t -> {});",
            "    Flux.empty().doOnComplete(() -> {});",
            "",
            "    // BUG: Diagnostic contains: Passing an on next signal `Consumer` on empty `Publisher` is a",
            "    // no-op",
            ...
            "    // BUG: Diagnostic contains: Passing an on next signal `Consumer` on empty `Publisher` is a",
            "    // no-op",
            "    Mono.just(22).then().subscribe(System.out::println);",
            "  }",
            "}")
        .doTest();
  }
