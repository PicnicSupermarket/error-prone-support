...
public final class TypeMemberOrder extends BugChecker implements CompilationUnitTreeMatcher {
    ...
    for (Tree tree : compilationUnitTree.getTypeDecls()) {
      if (tree instanceof ClassTree classTree) {
        suggestedFixes.merge(
            matchClass(classTree, requireNonNull(state.getSourceCode(), "Source code"), state));
      }
    }
  ...
  private SuggestedFix matchClass(ClassTree tree, CharSequence source, VisitorState state) {
    ...
    if (topLevelSorted) {
      ...
      for (TypeMember member : members) {
        if (member.tree() instanceof ClassTree memberClassTree) {
          nestedSuggestedFixes.merge(matchClass(memberClassTree, source, state));
        }
      }
     ...
     * fixes first, and only propagate their fixed source codes.
     */
    ImmutableMap.Builder<TypeMember, String> typeMemberSource = ImmutableMap.builder();
    for (TypeMember member : members) {
      if (member.tree() instanceof ClassTree memberClassTree) {
        SuggestedFix memberClassTreeFix = matchClass(memberClassTree, source, state);
        @Var
        String memberSource =
        ...
        for (Replacement replacement :
            memberClassTreeFix.getReplacements(
                ((JCCompilationUnit) state.getPath().getCompilationUnit()).endPositions)) {
          memberSource =
