...
run_id="$(
  gh workflow run "${workflow}" --ref "${branch}" -f pr_number="${pr_number}" -f dry_run="${dry_run}" \
    | grep -oP '(?<=https://github.com/PicnicSupermarket/error-prone-support/actions/runs/)\d+'
)"
